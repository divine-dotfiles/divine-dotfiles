...
#.  $D__DPL_ASSET_RELPATHS - (optional) Not a requirement, but populating this 
#.                          array with shortened (e.g., relative to assets dir) 
#.                          versions of the above is very helpful for debug 
#.                          output.
#.  $D__DPL_TARGET_PATHS   - This array must contain corresponding ‘target’ 
...
## Notes on automation:
#
## Framework provides a way to auto-populate asset paths ($D__DPL_ASSET_PATHS):
#.  * Asset manifest (see ‘dpl-filename.dpl.mnf’ template for reference)
#.        - This also populates $D__DPL_ASSET_RELPATHS array with shortened 
#.          versions of corresponding paths (relative to asset dir)
#
## Target paths ($D__DPL_TARGET_PATHS) can be populated manually. OS-specific 
#. overrides for this array are given by suffixing system handle in uppercase 
#. to variable name, e.g., $D__DPL_TARGET_PATHS_BSD. Browse ‘lib/adapters’ for 
...
#. out the rest.
#
## Target directory is provided in $D__DPL_TARGET_DIR variable. It is overridden 
#. for particular OS in the same manner as above, e.g., $D__DPL_TARGET_DIR_WSL.
#
