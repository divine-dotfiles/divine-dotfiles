...
__override_d_targets_for_distro()
{
  ...
  # Check if $D__DPL_TARGET_DIR_DEBIAN is not empty
  if [ -n "$D__DPL_TARGET_DIR_DEBIAN" ]; then

    # $D__DPL_TARGET_DIR_DEBIAN is set: use it instead
    D__DPL_TARGET_DIR=( "${D__DPL_TARGET_DIR_DEBIAN[@]}" )
