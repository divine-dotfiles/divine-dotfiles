...
{
  if [ ${#D_QUEUE_TARGETS_SOLARIS[@]} -gt 1 -o -n "$D_QUEUE_TARGETS_SOLARIS" ]
  then D_QUEUE_TARGETS=( "${D_QUEUE_TARGETS_SOLARIS[@]}" ); fi
  if [ -n "$D_QUEUE_TARGET_DIR_SOLARIS" ]
  then D_QUEUE_TARGET_DIR="$D_QUEUE_TARGET_DIR_SOLARIS"; fi
}