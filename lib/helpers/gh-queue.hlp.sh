...
d__gh_queue_pre_check()
{
  ...
  d__context -- push 'Preparing Github-queue for checking'; d__stash -- ready
...
d__gh_item_check()
{
  ...
  local d__gqei="$D__ITEM_NUM" d__gqrtc d__gqer=()
  local d__gqesk="gh_$( dmd5 -s "${D_DPL_TARGET_PATHS[$d__gqei]}" )"
  ...
  d__context -- push "Checking if copied to: $d__gqet"

  # Do the actual checking; check if source is readable; switch context
  if d__stash -s -- has $d__gqesk
  then [ -e "$d__gqet" ] && d__gqrtc=1 || d__gqrtc=6
  else [ -e "$d__gqet" ] && d__gqrtc=7 || d__gqrtc=2; fi
  if ! [ $d__gqrtc = 1 ]; then
    [ -e "$d__gqeb" ] && d__notify -l!h -- "Orphaned backup at: $d__gqeb"
  fi
  if ! [ -r "$d__gqea" ]; then
    d__notify -lxh -- "Unreadable asset at: $d__gqea"
    [ "$D__REQ_ROUTINE" = install ] && d__gqrtc=3
  fi
...
{
  # Storage varibales; switch context
  local d__gqei="$D__ITEM_NUM" d__gqrtc d__gqcmd
  local d__gqesk="gh_$( dmd5 -s "${D_DPL_TARGET_PATHS[$d__gqei]}" )"
  local d__gqet="${D_DPL_TARGET_PATHS[$d__gqei]}" d__gqetp
  local d__gqeb="$D__DPL_BACKUP_DIR/$d__gqesk"
  ...
  if d__push_backup -- "$d__gqet" "$d__gqeb"; then
    d__gqcmd=cp d__gqetp="$(dirpath -- "$d__gqet")"; if ! [ -w "$d__gqetp" ]
    then d__notify -u -- "Sudo privelege is required to operate under:" \
      -i- "$d__gqetp"; cmd='sudo cp'; fi
    if d__cmd $cmd -- --ASSET_PATH-- "$d__gqea" --TARGET_PATH-- "$d__gqet"
    then d__gqrtc=0; else d__gqrtc=1; fi
  else d__gqrtc=1; fi
  if [ $d__gqrtc -eq 0 ]; then
    case $D__ITEM_CHECK_CODE in
      2|7)  d__stash -s -- set $d__gqesk "$d__gqet" || d__gqrtc=1;;
    esac
...
{
  # Storage varibales; switch context
  local d__gqei="$D__ITEM_NUM" d__gqrtc
  local d__gqesk="gh_$( dmd5 -s "${D_DPL_TARGET_PATHS[$d__gqei]}" )"
  local d__gqet="${D_DPL_TARGET_PATHS[$d__gqei]}"
  local d__gqeb="$D__DPL_BACKUP_DIR/$d__gqesk"
  ...
  if d__pop_backup -e -- "$d__gqet" "$d__gqeb"
  then d__gqrtc=0; else d__gqrtc=1; fi
  if [ $d__gqrtc -eq 0 ]; then
    case $D__ITEM_CHECK_CODE in
      1|6)  d__stash -s -- unset $d__gqesk || d__gqrtc=1;;
    esac
