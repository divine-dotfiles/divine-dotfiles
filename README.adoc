...
:revnumber: 80
:revdate: 2019.09.18
:revremark: First stab at documenting workflow functions
:doctype: article
// Visual
:toc:
// Subs:
...
[source,subs="verbatim,quotes,attributes"]
----
*d{dus}context* [-l] [-t *_TITLE_*] [--] *push*|*pop*|*notch*|*lop* *_DESCRIPTION_*...
----
...
Most stack manipulations trigger a debug message that honors the global verbosity setting, as stored in <<var-opt-quiet,`$D{dus}OPT_QUIET`>>.
...
Prints the debug output regardless of the global verbosity setting.
...
[source,subs="verbatim,quotes,attributes"]
----
*d{dus}notify* [-1chlsuvx] [-t *_TITLE_*] [--] [*_DESCRIPTION_*...]
----

Whether the output is printed depends on the global verbosity setting, as stored in <<var-opt-quiet,`$D{dus}OPT_QUIET`>>.
...
Prints the notification regardless of the global verbosity setting.
This also causes a change in styling of the message.
...
In the `--loud` mode, colors the introductory arrow in green, if the terminal coloring is available.

| `*-x*`, `*--failure*`

In the `--loud` mode, colors the introductory arrow in red, if the terminal coloring is available.

| `*-s*`, `*--skip*`

In the `--loud` mode, colors the introductory arrow in white, if the terminal coloring is available.
...
Stashing is accessible from within the deployments via the `dstash` function.

[source,subs="verbatim,quotes,attributes"]
----
*dstash* [-drgsq] [--] [ *_CMD_* [ *_KEY_* [*_VALUE_*] ] ]
----
...
Without this option, every invocation of `dstash` (with any arguments) begins with stash readiness check.
If the function is called multiple times within a deployment, such checks become redundant.
It is recommended that after initial `dstash ready`, this option is then included with any subsequent invocations, to forego readiness checks.

| `*-q*`, `*--quiet*`

Without this option, any error during the stash readiness checks triggers an error message that always appears.
This option causes such error messages to instead honor the global verbosity setting.
