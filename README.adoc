...
:revnumber: 41
:revdate: 2019.08.22
:revremark: Add asset arrays section
:doctype: article
// Visual
:toc:
// Subs:
...
* You are greeted by `zsh` as your new default shell.
Unless you had it as default already.
...
* `*D_QUEUE_MAIN*` — for each path in the previous array, this one will contain a relative version of that path at the same index.
...
[NOTE]
--
When populating queue arrays, for each regex pattern in your asset manifest, the framework searches for matching files in <<var-dpl-asset-dir,deployment's asset directory>>.
This means, that the resulting queue will also include assets that might have been added by user manually, and not just ones provided with the deployment.
--
...
The rules are simple: append the handle of the supported OS in all capitals like this:
