...
| Attach pre-made deployments from Github (such as our glorious https://github.com/no-simpler/divine-bundle-essentials['essentials' bundle]) without polluting your precious Grail [.small]#_(heheh)_#
...
* `*bundles/*` — _Divine.dotfiles_ provides facilities to <<attach-routine,attach third-party deployments>> from Github.
...
* Directory for attached deployments: `~/.divine/state/bundles`.
...
Beside using own deployments, _Divine.dotfiles_ allows you to attach (i.e., import) third-party deployments distributed via Github repositories.
...
* `~/.divine/state/bundles` — attached third-party deployments.
