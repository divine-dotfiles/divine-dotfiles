...
When the dust settles, _voilà_:

* You will be greeted by `zsh` as the default shell.
* Both https://ohmyz.sh[`oh-my-zsh`] and https://github.com/Bash-it/bash-it[`bash-it`] frameworks will be installed and loaded.
(Existing installations will be re-used.)
* A minimalistic shell theme will be active in both frameworks.
* A few essential system packages will be available, e.g., `git`, `vim`, `curl`, etc.
* Opinionated default configs will be plugged in for `git`, `vim`, `bash`, and `zsh`.
* Divine utilites `dln`, `dmv`, and `dreadlink` will be available on `$PATH`.
...
| `*config-git*/`
| Global configuration files for `git`.
...
Notice that names of sub-directories inside `~/.divine/grail/assets/` match the names of their respective deployments.
...
* Directory for attached bundles of deployments: `~/.divine/state/bundles`.
...
* `~/.divine/state/bundles` — attached third-party bundles of deployments.
...
** `~/.divine/state/bundles` — attached third-party Divinefiles.
...
Watever assets are operated upon should be placed in the asset directory.
...
Contents of an asset manifest describe a set of *assets* (files and directories) by providing relative paths to them.
Whenever an asset manifest is processed, the framework:

* looks up the assets in both <<var-dpl-dir,deployment directory>> (deployment author's versions) and <<var-dpl-asset-dir,asset directory>> (deployment user's versions);
* copies author's initial versions into asset directory, where necessary;
* populates <<asset-arrays,global asset arrays>> that can then be used by <<generic-queue,queues>> of <<link-queue,various>> <<copy-queue,types>>.

[NOTE]
--
The framework never overwrites assets that are already present in asset directory.
--
...
Two types of paths are accepted: *concrete paths* and *RegEx patterns*.
Leading and trailing slashes are removed from all paths in the asset manifests (including <<asset-manifest-prefix,`prefix`>> sub-paths).
...
Asset manifests are designed for convenience, not security.
--
...
[NOTE]
--
Normally, if you wish to use any <<link-queue-asset-automation,queue>> <<copy-queue-asset-automation,automation>>, you must use it only for the very first task that employs a queue, and append queue elements manually from that point on.

However, if you use <<asset-manifests,asset manifests>>, you may <<asset-manifest-queue-split,split your queue>> directly within the asset manifest file.
--
