...
. *Primary intervention routines* on deployments (and Divinefiles):
...
=== Primary intervention routines

[source,bash,subs="verbatim,attributes,quotes"]
----
$ *di* *c*|*check*    [-ynqvewf] [--] [*_NAME_*]…

$ *di* *i*|*install*  [-ynqvewf] [--] [*_NAME_*]…

$ *di* *r*|*remove*   [-ynqvew]  [--] [*_NAME_*]…
----

Accepted `*_NAME_*` values are (case-insensitive):
...
Deployments (and Divinefiles) are retrieved from two locations (any depth):
...
Filtering patterns for deployments work as follows:

* Without any names, all deployments are processed.
* Particular set of deployments is requested by listing names or <<deployment_groups,single-digit group names>>, in any combination.
...
* Option `--except`/`-e` inverts filtering: all deployments are processed, except those listed.
Note, that without any names, this is a no-opt.
+
...
[NOTE]
--
Even though every option above serves a function within the framework, it is also up to deployment authors to honor their semantic meaning.
--
...
Attached deployments are stored in 