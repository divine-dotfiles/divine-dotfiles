...
:revdate: 2019.10.31
:revremark: Update readme for D.d v2, pt. 3
:doctype: article
// Visual
:toc:
// Subs:
...
On a new machine — <<plug-routine,plug in>> your Grail from a repo or a local dir with one command.
...
Deployments support basic grouping via single-character <<deployment-flags,flags>>.
...
However, the framework will bug you with suggestions to auto-install it until you relent.
|===
...
[%noheader,cols="<.<a"]
|===
| Nothing too spectacular will happen:

* Optional dependencies — such as Git — will be offered, unless installed already.
* This repository will be cloned/downloaded.
* Optionally, one symlink (<<intervention-utility,`di`>>) to the framework's main script will be created on `$PATH`.
* Zero data loss <<fmwk-zero-data-loss,policy>> will be in effect, as normal.

And naturally, you will be <<fmwk-interactivity,prompted>> for everything.
|===

The documentation assumes that the framework has been installed with the optional shortcut command <<intervention-utility,`di`>>.
...
This directs to install it into the `_DIRPATH_` directory instead.

| `*D_SHCT_NAME*=_CMD_`

By default, the shortcut command for the framework is named `di`.
This directs to name it `_CMD_` instead.

| `*D_SHCT_DIR*=_DIRPATH_`

By default, the shortcut command for the framework is installed into one of the usual `/{das}/bin/` directories on `$PATH`.
This directs to install it into the `_DIRPATH_` directory instead.
...
Assumes an affirmative answer to the prompts about installing the framework and all optional parts.

| `*-n*`, `*--no*`

Assumes a negatory answer to the prompts about installing the framework and all optional parts.
In essence, this performs a harmless 'dry run'.

| `*-d*`, `*--fmwk-yes*`

Assumes an affirmative answer to the prompt about installing the framework.

| `*-D*`, `*--fmwk-no*`

Assumes a negatory answer to the prompt about installing the framework.
This option is effectively a synonym of `--no`, as no optional parts are installed without the framework itself.

| `*-s*`, `*--shct-yes*`

Assumes an affirmative answer to the prompt about installing the shortcut command.

| `*-S*`, `*--shct-no*`

Assumes a negatory answer to the prompt about installing the shortcut command.

| `*-f*`, `*--force*`

By default the installation halts immediately if the destination path already exists and is not an empty directory.
This is in keeping with the zero data loss <<fmwk-zero-data-loss,policy>>.
This option directs to instead issue an <<urgent-prompts,urgent prompt>> about whether to displace the pre-existing destination to a backup location.

| `*-v*`, `*--verbose*`

_(repeatable)_
Gradually increases the amount of informative output.

This option is fully analogous to the <<global-verbosity-level,global verbosity level>> used by the framework itself.

| `*-q*`, `*--quiet*`

Resets the amount of output to the default, minimal level.
...
And, again, you will be <<fmwk-interactivity,prompted>> for everything.
|===
...
This directs to uninstall it from the `_DIRPATH_` directory instead.

^.^h| Append on the right

| `*-y*`, `*--yes*`

Assumes an affirmative answer to the prompts about uninstalling the framework and all optional parts.

| `*-n*`, `*--no*`

Assumes a negatory answer to the prompts about uninstalling the framework and all optional parts.
In essence, this performs a harmless 'dry run'.

| `*-d*`, `*--fmwk-yes*`

Assumes an affirmative answer to the prompt about uninstalling the framework.

| `*-D*`, `*--fmwk-no*`

Assumes a negatory answer to the prompt about uninstalling the framework.
This option is effectively a synonym of `--no`, as no optional parts are uninstalled without the framework itself.

| `*-u*`, `*--util-yes*`

Assumes an affirmative answer to the prompt about uninstalling the optional dependencies that might have been installed.

| `*-U*`, `*--util-no*`

Assumes a negatory answer to the prompt about installing the optional dependencies that might have been installed.

| `*-f*`, `*--force*`

By default the uninstallation halts immediately if it fails to uninstall any of the optional dependencies.
This option directs to instead issue an <<urgent-prompts,urgent prompt>> about whether to ignore the debacle.

| `*-o*`, `*--obliterate*`

By default the uninstallation of the framework means displacing its directory to a backup location.
This is in keeping with the zero data loss <<fmwk-zero-data-loss,policy>>.
The `--obliterate` option directs to irrecoverably erase the framework directory instead.
...
[%noheader,cols="<.<a"]
|===
| This three-part command is pretty unceremonious:
...
For this joy ride, no framework <<fmwk-interactivity,prompts>> will be issued.
|===

As stated before, the Divine deployments <<fmwk-zero-data-loss,do not overwrite>> pre-existing files on your system without backing them up.
...
(On macOS, you have to re-log into the system.)
...
Within the `.runcoms` sub-directory, all files ending with `.sh`, `.bash`, and `.zsh` are sourced in order on every shell start-up.
...
[source,bash]
----
# Uninstall the Divine deployments WITHOUT BACKUPS:
~/.divine/intervene.sh remove --yes --obliterate

# Detach the essentials bundle from the Grail directory:
~/.divine/intervene.sh detach essentials --yes

# Erase the framework WITHOUT BACKUPS:
bash -c "TMP=\$(mktemp); \
URL=https://raw.github.com/no-simpler/divine-dotfiles/master/lib/uninstall/uni\
nstall.sh; if curl --version &>/dev/null; then curl -fsSL \$URL >\$TMP; elif w\
get --version &>/dev/null; then wget -qO \$TMP \$URL; else printf >&2 \"\n==> \
...
After the three 'undo' steps have successfully run, there is no trace of _Divine.dotfiles_ on the system.
[.small]#_(Sigh.)_#

== Framework structure

_Divine.dotfiles_ is installed, by default, to the `~/.divine/` directory, and is contained entirely in that directory, *except*:

* A symlink to the framework's main script is created somewhere on `$PATH`.
* Deployments may affect your system pretty much <<fmwk-security,anywhere>>.
