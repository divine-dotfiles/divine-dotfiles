...
Located within <<grail_directory,the Grail>>, specifically `grail/assets/*_DPL-NAME_*/`.

| `*D__DPL_BACKUP_DIR*`
| Generated absolute path to directory assigned to hold backups of current deployment.

Located within <<state_directory,state directory>>, specifically `state/backups/*_DPL-NAME_*/`.
...
Same as path to `\*.dpl.sh` file, but with suffix changed to `*.dpl.mnf`.

[NOTE]
--
Asset manifests are also processed by other internal routines, which don't source deployments.

Consequently, path schema of asset manifests is locked, and this variable is read-only.
--
...
Same as path to `\*.dpl.sh` file, but with suffix changed to `*.dpl.que`.

[NOTE]
--
Queue manifests are processed only after their deployment file is sourced.

Consequently, you are free to adjust this path.
--
...
Check for both to determine whether currently under Linux, e.g.:
...
If `$D_DPL_NEEDS_ANOTHER_PROMPT` is set to `true` and this variable is non-empty, then this textual warning is shown to the user.
