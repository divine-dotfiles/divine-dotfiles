...
:revdate: 2019.11.01
:revremark: Update readme for D.d v2, pt. 4
:doctype: article
// Visual
:toc:
// Subs:
...
On a new machine — the Grail can be <<plug-routine,plugged in>> from a repo or a local dir with one command.
...
* `*bundles*/` — The directory for the copies of <<attach-routine,attached>> bundles of deployments.
...
Assumes an affirmative answer to the prompt about installing the <<shortcut-cmd,shortcut command>>.

| `*-S*`, `*--shct-no*`

Assumes a negatory answer to the prompt about installing the <<shortcut-cmd,shortcut command>>.

| `*-f*`, `*--force*`

By default the installation halts immediately if the destination path already exists and is not an empty directory.
This is in keeping with the non-interference <<fmwk-non-interference,policy>>.
The `--force` option directs to instead issue an <<urgent-prompts,urgent prompt>> about whether to displace the pre-existing destination to a backup location.
...
The `--verbose` option is fully analogous to the <<global-verbosity-level,global verbosity level>> used by the framework itself.
...
The `--force` option directs to instead issue an <<urgent-prompts,urgent prompt>> about whether to ignore the debacle.
...
The following single shell command methodically undoes all the <<joy-ride,joy ride>> installation:
