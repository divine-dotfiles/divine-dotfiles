...
* The special variable `*D_QUEUE_TARGET_DIR*` is set to a path to some target directory.
...
The *auto-targeting* simply creates the `*D_QUEUE_TARGETS*` array, in which every relative path from the `D_QUEUE_MAIN` array is prepended with the `D_QUEUE_TARGET_DIR` directory path.

The auto-targeting can be used for the <<spcqe-main,specialized queues>>, which make use of the `D_QUEUE_TARGETS` array.
Naturally, the auto-targeting only works when all the relarive targets are grouped in the same directory.

Different targets can be given for the different operating systems by appending the `{us}**_OS_**` suffix to the name of the variable.
 The `*_OS_*` is the name of the OS <<indct-or-family,family>> or <<indct-or-distro,distro>> in all caps.
 This works for both the `D_QUEUE_TARGETS` and the `D_QUEUE_TARGET_DIR` variables.
 For example:

* On the `bsd` OS family, the overriding variable for the targers is named `D_QUEUE_TARGETS_BSD`.
* On the `debian` distribution, the overriding variable for the target directory is named `D_QUEUE_TARGET_DIR_DEBIAN`

The distribution override takes precedence over the OS family override.
...
** `**d{dus}**name` — the functions to be called, e.g., the <<stash,`d__stash`>> function.
...
populate_link_queue() {
  ...
  D_QUEUE_ASSETS=( "$src/.bashrc" "$src/.zshrc" )
  D_QUEUE_TARGETS=( $dst/.bashrc   $dst/.zshrc  )
}
----
...
In the context of link queue, for each absolute path in `*D_QUEUE_ASSETS*`, this array must contain a shortened relative version, resolvable from the asset directory.
...
For each path in `*D_QUEUE_ASSETS*`, this array must contain the intended absolute path to a symlink, which will point to the respective asset.
...
* Arrays `*D_QUEUE_MAIN*` and `*D_QUEUE_ASSETS*` are both <<assets-arrays,automatically populated>> when the <<assets-mnf,asset manifest>> is processed, which happens immediately before your deployment file is sourced.
* Array `*D_QUEUE_TARGETS*` can be automatically populated if all your assets are intended to go into one target directory.
+
For that to occur, put the target directory into `*D_QUEUE_TARGET_DIR*` variable before any of the built-in helpers are called.
...
In _Divine.dotfiles_, every OS adapter carries an override mechanism for both `*D_QUEUE_TARGET_DIR*` and `*D_QUEUE_TARGETS*`.
The rules are simple: append the handle of the supported OS to the variable name in all capitals like this:

[source,bash]
----
D_QUEUE_TARGET_DIR=/generic/path
D_QUEUE_TARGET_DIR_MACOS=/path/on/macos
D_QUEUE_TARGET_DIR_FEDORA=/path/on/fedora
D_QUEUE_TARGET_DIR_WSL=/path/on/wsl
----
...
populate_copy_queue() {
  ...
  D_QUEUE_ASSETS=( "$src/font1.ttf" "$src/font2.ttf" )
  D_QUEUE_TARGETS=( $dst/font1.ttf   $dst/font2.ttf  )
}
----
...
This variable is still the main definition of the queue.
In the context of copy queue, for each absolute path in `*D_QUEUE_ASSETS*`, this array must contain a shortened relative version, resolvable from the asset directory.
...
For each path in `*D_QUEUE_ASSETS*`, this array must contain the destination absolute path for copying.
...
* Arrays `*D_QUEUE_MAIN*` and `*D_QUEUE_ASSETS*` are both <<asset-arrays,automatically populated>> when the <<assets-mnf,asset manifest>> is processed, which happens immediately before your deployment file is sourced.
* Array `*D_QUEUE_TARGETS*` can be automatically populated if all your assets are intended to go into one target directory.
+
For that to occur, put the target directory into `*D_QUEUE_TARGET_DIR*` variable before any of the built-in helpers are called.
