...
:revnumber: 31
:revdate: 2019.08.21
:revremark: First complete revision
:doctype: article
// Visual
:toc:
// Subs:
...
[NOTE]
--
This list is incomplete; you can help by <<contributing_os_support,expanding it>>.
--
...
* `*g*` or `*grail*` — pulls latest revision of <<grail_directory,Grail directory>>, _if_ it is a <<plugging_the_grail,plugged>> repository.
* `*d*` or `*deployments*` — pulls latest revision of every <<attaching_deployments,_attached_>> deployment repository.
* Without any arguments, all of the above are performed.

==== Updating routine options

Below is the list of options for updating framework, <<grail_directory,the Grail>>, and <<attaching_deployments,attached deployments>>.
Additional info about routine's behavior is also given.
...
Divinefiles follow the general <<manifest_syntax,manifest syntax>>.

Every entry is a *list* of whitespace-separated names of packages.
Keys `flags` and `priority` set the respective attributes for the packages, and work identically to <<deployment_flags,flags>> and <<deployment_priority,priority>> in regular deployments.
...
It is good style to isolate all deployment logic within functions and global variables, and then call/use them within <<primary_functions,primary functions>>.
...
2+^.^h| <<deployment_metadata,Deployment metadata>>
...
| Works only during `install`/`remove` <<primary_routines,routine>> and only if set within `d_dpl_check()` primary.
...
| `*D__REQ_ROUTINE*`
a| Name of <<primary_routines,primary routine>> currently being executed:
...
[NOTE]
--
Call `dprompt --answer "$D__OPT_ANSWER"` to make the prompt honor `--yes`/`--no` options of the <<intervention_utility,intervention utility>>.
--
...
* During <<primary_routines,primary routines>>, after sourcing deployment file, but before calling any of the <<primary_functions,primary functions>>.
...
Asset manifests follow the general <<manifest_syntax,manifest syntax>>.
...
To be interpreted as a regex pattern, a path must be <<manifest_entry_flags,flagged>> with `r` character.
...
. Delegate your deployment's <<primary_functions,primaries>> to built-in helpers.
...
Unlike with <<asset_manifests,asset manifests>>, you are free to customize the location/name of your queue manifest by re-assigning <<var_dpl_que_path,`D_DPL_QUE_PATH`>> variable *at the top level* of your deployment.

Queue manifests follow the general <<manifest_syntax,manifest syntax>>.
...
. Declare the queue manifest an asset, by listing it in your <<asset_manifests,asset manifest>>.
...
For such purposes, _Divine.dotfiles_ provides a partially implemented version of <<generic_queue,generic queue>> called *link queue*.
To use it:
...
Following Bash arrays should be populated before any of the built-in helpers are called.
Note, that you can <<link_queue_asset_automation,automate>> this process.
...
However, for a narrow use case, when you employ <<link_queue_asset_automation,link queue automation>> without the <<asset_manifests,asset manifest>>, you will be required to populate this array manually.
...
[[link_queue_asset_automation]]
==== Link queue asset automation

So, you have your list of assets, stored in a <<asset_manifests,manifest>>.
...
For such purposes, _Divine.dotfiles_ provides a partially implemented version of <<generic_queue,generic queue>> called *copy queue*.
To use it:
...
If you want to actually substitute existing files (while backing them up), prefer the <<link_queue,link queue>>.
--
...
Following Bash arrays should be populated before any of the built-in helpers are called.
Note, that you can <<copy_queue_asset_automation,automate>> this process.
...
However, for a narrow use case, when you employ <<copy_queue_asset_automation,copy queue automation>> without the <<asset_manifests,asset manifest>>, you will be required to populate this array manually.
...
[[copy_queue_asset_automation]]
==== Copy queue asset automation

Copy queue asset automation works identically to <<link_queue_asset_automation,that of link queue>>.
...
[NOTE]
--
If you wish to use any <<link_queue_asset_automation,queue>> <<copy_queue_asset_automation,automation>>, you must use it only for the very first task that employs a queue.
...
[[contributing_os_support]]
=== Contributing OS support

One of the main goals of this framework is *portability*.

Within _Divine.dotfiles_, every supported operating system is represented with a special file called an *adapter*.
A distribution adapter allows the framework to interact with the OS and with its system package manager.

* An adapter is a Bash script similar to an implemented interface.
* Guidelines for adapter implementation are located in `lib/templates/adapters/distro.adp.sh`.
* Active adapters are put into `lib/adapters/distro`.

Working adapters for all operating systems out there will be welcomed with open arms.