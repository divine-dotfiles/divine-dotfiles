...
_Divine.dotfiles_ offers a pill that relieves such pain.

==== Generic queue

A *queue* is exactly what it sounds like:

. Populate a specially named array with one string for each queue item.
. Implement logic to be applied to a single item.
. Delegate your deployment's <<_primary_functions,primaries>> to built-in versions.

This kind of thing is best demonstrated on an example:

.Deployment template for generic queue
[source,bash]
----
d_dpl_check()    { populate_queue;  d__queue_check;    }  # <1>
d_dpl_install()  {                  d__queue_install;  }
d_dpl_remove()   {                  d__queue_remove;   }

populate_queue() { D_QUEUE_MAIN=( alpha bravo charlie ); }  # <2>

d_queue_item_check()    { :; }  # <3>
d_queue_item_install()  { :; }
d_queue_item_remove()   { :; }
----
<1> Delegate primaries to built-in queue helpers.
Make sure helper is called last.
<2> This function is not built-in, but is an example of code organization.
<3> Implement mini-primaries for a single queue item
