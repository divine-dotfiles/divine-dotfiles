...
:revdate: 2019.09.25
:revremark: Describe new behavior of d__cmd and siblings
:doctype: article
// Visual
:toc:
// Subs:
...
_(repeatable, verbosity option)_
...
If no verbosity options are given, than the default quiet levels for the commands of this function are:
...
If no verbosity options are given, than the default quiet level for this function is `1`.
...
_(repeatable, verbosity option)_
If at least one such option is provided, then the amount of them designates the function's *quiet level*.
Whatever output of the `*_CMD_*` is _not_ suppressed is printed only if the <<global-verbosity-level,global verbosity level>> is greater than or equal to the function's quiet level.

If no verbosity options are given, than the default quiet level for this function is `0`.

| `*--l--*`

_(verbosity option)_
Sets the quiet level to zero.
In effect this will cause the non-suppressed output to be printed regardless of the <<global-verbosity-level,global verbosity level>>.

Options are read left-to-right: if both verbosity options are mixed together, the total quiet level is the amount of `--q--` options after the last `--l--` option.
