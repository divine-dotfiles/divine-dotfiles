...
:revdate: 2019.10.31
:revremark: Update readme for D.d v2, pt. 2
:doctype: article
// Visual
:toc:
// Subs:
...
A special kind of deployments that maintains a stable set of packages across machines and OS's.
...
A glaring exception to this is the upgrading of the local package repositories through the system package manager (e.g., `sudo dnf upgrade -y`), which is deemed safe enough to overlook.
...
|===
+
The framework will work on other operating systems too, but without support for packages (e.g., <<divinefiles,Divinefiles>> will not work).
...
Still, all invocations of the `di` command can be substituted by executing the `./intervene.sh` script in the root of the installation directory.
...
By default the installation halts immediately if the destination directory already exists and is not empty.
This option allows to instead back up the pre-existing destination via an <<urgent-prompts,urgent prompt>>.
...
Directing an existing installation of _Divine.dotfiles_ to uninstall itself is also possible:
...
By default the uninstallation halts immediately if it fails to uninstall any of the existing optional dependencies.
This option allows to instead ignore such debacles via an <<urgent-prompts,urgent prompt>>.
