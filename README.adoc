...
| [#var_queue_main]#`*D_DPL_ASSET_PATHS*`#
...
| [#var_queue_main]#`*D_DPL_ASSET_RELPATHS*`#
| [.gray]##_(optional)_## For each absolute path in `*D_DPL_ASSET_PATHS*`, this array may contain a shortened version, relative to the asset directory.

These shortened versions are used only in debug messages, to make them terser.

| [#var_queue_main]#`*D_DPL_TARGET_PATHS*`#
...
Eventually, you will want to include a couple of queues in your multitask deployment.
And therein lies a big _gotcha_.
...
d_queue1_check()    { populate_copy_queue;  d__copy_queue_check;    }
d_queue1_install()  {                       d__copy_queue_install;  }
d_queue1_remove()   {                       d__copy_queue_remove;   }

d_queue2_check()    { populate_link_queue;  d__link_queue_check;    }
d_queue2_install()  {                       d__link_queue_install;  }
d_queue2_remove()   {                       d__link_queue_remove;   }

d_queue3_check()    { populate_queue;       d__queue_check;         }
d_queue3_install()  {                       d__queue_install;       }
d_queue3_remove()   {                       d__queue_remove;        }
...
populate_link_queue() {
  # In subsequent queues, call this function first
  *d__queue_skip*

  # Then *append* to the queue array
  D_QUEUE_MAIN=( some copy tasks)
}
----
